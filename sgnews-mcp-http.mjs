...
const API_BASE = (process.env.SGNEWS_API_BASE || "https://news.booksystems.co.uk/api").replace(/\/$/, "");
...
const server = new McpServer({ name: "sg-news", version: "1.2.0" });

server.registerTool(
  "search_articles",
  {
    title: "Search SG News",
    description: "Search recent SG News articles before deciding whether to create a new article or update an existing one.",
    inputSchema: {
...
server.registerTool(
  "hide_article",
  {
    ...
    description: "Hide an SG News article from the public site without permanently deleting its record.",
    inputSchema: { slug: z.string().min(1) }
  },
  async ({ slug }) => asText(await api("/articles/" + encodeURIComponent(slug), {
    method: "DELETE"
